...
exports.getVolunteerByUserNameAndPassword = function (userName, password) {
  return new Promise((resolve) => {
    tp.sql(
      "SELECT [record_id] ,[first_name] ,[last_name] ,[username],[role],[email], [status] " +
        " FROM [dbo].[Volunteers] where username= '" +
        userName +
        "' and password=HashBytes('MD5', '" +
        password +
        "') and status = 'Active'"
    )
      .execute()
...
exports.getVolunteerByUserNameAndPassword = function (userName, password) {
    return new Promise((resolve) => {
        var sql = "SELECT [record_id] ,[first_name] ,[last_name] ,[username],[role],[email], [status] " + " FROM [dbo].[Volunteers] where username= '" + userName + "' and password=HashBytes('MD5', '" + password + "') and status = 1";
...
<<<<<<< Updated upstream
  ...
  pool.acquire(function (err, connection) {
    ...
    var request = new Request(
      "INSERT INTO [dbo].[Volunteers] ([first_name], [last_name], [username], [password], [home_phone], [work_phone], [cell_phone], [email], [educational_background], " +
    ...
    request.addParameter(
      "DRIVER_LICENSE",
      TYPES.Bit,
      userObject.driversLicense
    );
    request.addParameter(
      "SOCIAL_SECURITY",
      TYPES.Bit,
      userObject.socialSecurity
    );
    request.addParameter(
      "EMERGENCY_FIRST_NAME",
      TYPES.VarChar,
      userObject.emergencyFirstName
    );
    request.addParameter(
      "EMERGENCY_LAST_NAME",
      TYPES.VarChar,
      userObject.emergencyLastName
    );
    request.addParameter(
      "EMERGENCY_EMAIL",
      TYPES.VarChar,
      userObject.emergencyEmail
    );
    request.addParameter(
      "EMERGENCY_PHONE",
      TYPES.VarChar,
      userObject.emergencyPhone
    );
    request.addParameter(
      "EMERGENCY_ADDRESS",
      TYPES.VarChar,
      userObject.emergencyAddress
    );
...
exports.getAllVolunteersBySearchValue = function (searchValue) {
  return new Promise((resolve) => {
    var sql =
      "SELECT * FROM [ProjectZero].[dbo].[Volunteers] where first_name like '%'+'" +
      searchValue +
      "'+ '%' or last_name like '%'+'" +
      searchValue +
      "' + '%'";
...
exports.getAllBusinesses = function () {
  return new Promise((resolve) => {
    tp.sql("SELECT * FROM [dbo].[Businesses]")
      .execute()
      .then(function (results) {
        console.log(results);
        resolve(results);
...
exports.createNewBusiness = function (businessObject) {
  ...
  pool.acquire(function (err, connection) {
    ...
    //use the connection as normal
    var request = new Request(
      "INSERT INTO [dbo].[Businesses] ([business_name],[email],[primary_contact_fName],[primary_contact_lName], " +
        "[primary_contact_phone_number],[secondary_contact_fName],[secondary_contact_lName],[secondary_contact_phone_number]," +
        "[address],[Services_Offered],[Service_Area],[Discount_Amount],[Preferred_Method_Contact],[EOY_Receipt]) " +
        "VALUES (@business_name , @email, @primary_contact_fName, @primary_contact_lName, @primary_contact_phone_number," +
        "@secondary_contact_fName,@secondary_contact_phone_number," +
        "@address,@Services_Offered,@Service_Area,@Discount_Amount,@Preferred_Method_Contact,@EOY_Receipt);",
      function (err, rowCount) {
        if (err) {
          console.error(err);
    ...
    request.addParameter(
      ...
      TYPES.VarChar,
      businessObject.primary_contact_fName
    );
    request.addParameter(
      "primary_contact_lName",
      TYPES.VarChar,
      businessObject.primary_contact_lName
    );
    request.addParameter(
      "primary_contact_phone_number",
      TYPES.VarChar,
      businessObject.primary_contact_phone_number
    );
    request.addParameter(
      "secondary_contact_fName",
      TYPES.VarChar,
      businessObject.secondary_contact_fName
    );
    request.addParameter(
      "secondary_contact_lName",
      TYPES.VarChar,
      businessObject.secondary_contact_lName
    );
    request.addParameter(
      "secondary_contact_phone_number",
      TYPES.VarChar,
      businessObject.secondary_contact_phone_number
    );
    request.addParameter(
      "Services_Offered",
      TYPES.VarChar,
      businessObject.Services_Offered
    );
    request.addParameter(
      "Service_Area",
      TYPES.VarChar,
      businessObject.Service_Area
    );
    request.addParameter(
      "Preferred_Method_Contact",
      TYPES.VarChar,
      businessObject.Preferred_Method_Contact
    );
    request.addParameter(
      "EOY_Receipt",
      TYPES.VarChar,
      businessObject.EOY_Receipt
    );
...
exports.updateVolunteer = function (userObject) {
    pool.acquire(function (err, connection) {
        ...
        // use the connection as normal
        var request = new Request("UPDATE [dbo].[Volunteers] " + "SET [first_name] = @FIRST_NAME, [last_name] = @LAST_NAME, [username] = @USER_NAME, [home_phone] = @HOME_PHONE, [work_phone] = @WORK_PHONE, [cell_phone] = @CELL_PHONE, " + "[email] = @EMAIL, [educational_background] = @EDUCATION, [current_licenses] = @LICENSES, [availability] = @AVAILABILITY, [role] = @ROLE, [status] = @STATUS, " + "[emergency_contact_name] = @EMERGENCY_FIRST_NAME, [emergency_contact_phone] = @EMERGENCY_PHONE, [emergency_contact_email] = @EMERGENCY_EMAIL, [emergency_contact_address] = @EMERGENCY_ADDRESS " + "WHERE [record_id] = @ID", function (err, rowCount) {
            if (err) {
                console.error(err);
                return;
            }
...
exports.changeVolunteerPassword = function (passwordHash, volunteerId) {
    pool.acquire(function (err, connection) {
        ...
        // use the connection as normal
        var request = new Request("UPDATE [dbo].[Volunteers] SET [password] = HashBytes('MD5', @PASSWORD)" + "WHERE [record_id] = @ID", function (err, rowCount) {
            if (err) {
                console.error(err);
                return;
            }
...
exports.getAllVolunteersBySearchValue = function (searchValue) {
    return new Promise((resolve) => {
        var sql = "SELECT * FROM [dbo].[Volunteers] where first_name like '%'+'" + searchValue + "'+ '%' or last_name like '%'+'" + searchValue + "' + '%'";
        console.log(sql);
        tp.sql(sql).execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
...
exports.getVolunteerById = function (volunteerId) {
    return new Promise((resolve) => {
        tp.sql("SELECT * FROM [dbo].[Volunteers] where record_id = " + volunteerId).execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
        });
...
exports.getVolunteerByEmail = function (volunteerEmail) {
    return new Promise((resolve) => {
        tp.sql("SELECT * FROM [dbo].[Volunteers] where email = '" + volunteerEmail + "'").execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
        });
...
exports.getAllEducations = function () {
    return new Promise((resolve) => {
        tp.sql("SELECT * FROM [dbo].[Education_Levels]").execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
        });
...
exports.getAllRoles = function () {
    return new Promise((resolve) => {
        tp.sql("SELECT * FROM [dbo].[Roles]").execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
        });
...
exports.getAllBusinesses = function () {
    return new Promise((resolve) => {
        tp.sql("SELECT * FROM [dbo].[Businesses]").execute().then(function (results) {
            console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
...
exports.createNewBusiness = function (businessObject) {
    ...
    pool.acquire(function (err, connection) {
        ...
        // use the connection as normal
        var request = new Request("INSERT INTO [dbo].[Businesses] ([business_name], [email], [primary_contact_fName], [primary_contact_lName], [primary_contact_phone_number],[secondary_contact_fName],[secondary_contact_lName],[secondary_contact_phone_number],[address],[Services_Offered],[Service_Area],[Discount_Amount],[Preferred_Method_Contact],[EOY_Receipt])" + "VALUES (@business_name , @email, @primary_contact_fName, @primary_contact_lName, @primary_contact_phone_number, @secondary_contact_fName, @secondary_contact_phone_number, @address, @Services_Offered, @Service_Area, @Discount_Amount, @Preferred_Method_Contact, @EOY_Receipt);", (err, rowCount) => {
        ...
        request.addParameter("Preferred_Method_Contact", TYPES.VarChar, businessObject.Preferred_Method_Contact);
        request.addParameter("EOY_Receipt", TYPES.VarChar, businessObject.EOY_Receipt);
        //request.addParameter("record_id", TYPES.VarChar, businessObject.record_id);
        connection.execSql(request);
    });
    return 1;
>>>>>>> Stashed changes

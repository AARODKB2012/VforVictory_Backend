...
connection.execSql(request);});// Returning one if no error occurred.return 1;};
...
exports.getAllBudgets = function () {
  ...
  exports.createNewBudget = function (budgetObj) {
    ...
    pool.acquire(function (err, connection) {
      ...
      // use the connection as normal
      var request = new Request(
        "INSERT INTO [dbo].[Budget] ([id],[amount],[start_date],[finish_date],[family_id],[current_balance]) " +
        "VALUES (@ID ,@AMOUNT ,@START_DATE ,@FINISH_DATE ,@FAMILY_ID ,@CURRENT_BALANCE);",
        function (err, rowCount) {
          if (err) {
            console.error(err);
            return;

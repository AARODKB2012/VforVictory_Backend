...
exports.getVolunteerByUserNameAndPassword = function (userName, password) {
    return new Promise((resolve) => {
        tp.sql("SELECT [record_id] ,[first_name] ,[last_name] ,[username],[role],[email], [status] " + " FROM [dbo].[Volunteers] where username= '" + userName + "' and password=HashBytes('MD5', '" + password + "') and status = 'Active'").execute().then(function (results) { // console.log(results);
            resolve(results);
        }).fail(function (err) {
            console.log(err);
        });
...
exports.createNewBusiness = function (businessObject) {
    ...
    pool.acquire(function (err, connection) {
        ...
        request.addParameter("business_name", TYPES.VarChar, businessObject.firstName);
